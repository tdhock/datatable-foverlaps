...
for(strand in names(full.strand.list)){
  ...
  stopifnot(nrow(overlap.dt) == nrow(overlap.chipseq))
  stopifnot({
    all.equal(as.character(overlap.dt$chrom),
              as.character(overlap.chipseq$seqnames))
  })
  stopifnot({
    all.equal(overlap.dt$i.chromStart, overlap.chipseq$start)
  })
  stopifnot(all.equal(overlap.dt$i.chromEnd, overlap.chipseq$end))
  stopifnot(all.equal(overlap.dt$count, overlap.chipseq$coverage))
  
  stopifnot(nrow(overlap.dt) == nrow(overlap.windows))
  stopifnot(all.equal(as.character(overlap.dt$chrom),
                      as.character(overlap.windows$seqnames)))
  stopifnot(all.equal(overlap.dt$chromStart, overlap.windows$start))
  stopifnot(all.equal(overlap.dt$chromEnd, overlap.windows$end))
